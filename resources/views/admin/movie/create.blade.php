@extends('admin.layouts.form', [
    'action' => route('movie.store', $movie),
    'method' => 'POST',
...
{{--    <script src="node_modules/vue-filepond/dist/vue-filepond.min.js"></script>--}}
@endsection
                ...
                success : function(response){
                    ...
                    if (response.length > 0) {
                        $.each(response, function (index, item) {
                            output = "<div class='result-item clearfix' data-item-id='"+ item.id +"' data-item-name='"+ item.name +"' data-item-image='"+ item.featured_image.thumb_path +"'>";

                            if (item.featured_image) {
                                output += "<div class='result-item__avatar'><img src='" + item.featured_image.thumb_path + "' /></div>";
